...
export class EventForwarder {
    ...
    logger;
    lastToolAt = new Map();
    constructor(ctx, bot, state, pending, queue, onPendingClear, htmlFormatting = true, statusLineEnabled = true, logger) {
        this.ctx = ctx;
        this.bot = bot;
        this.state = state;
    ...
    findChatId(sessionId) {
        ...
        for (const [chatId, chat] of Object.entries(state.chats)) {
            ...
            if (chat.sessions?.some((entry) => entry.sessionId === sessionId))
                return Number(chatId);
        }
        return undefined;
    }
    async sendToTelegram(chatId, text) {
        ...
        const isLastChunk = (index) => index === chunks.length - 1 && showActions;

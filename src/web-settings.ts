...
function csrfTokenFor(botToken: string, windowStart: number): string {
  return createHmac('sha256', csrfSecret).update(`${botToken}:${windowStart}`).digest('hex');
}

function csrfWindows(now = Date.now()): number[] {
  const window = Math.floor(now / CSRF_WINDOW_MS);
  return [window, window - 1]; // 当前 + 前一窗口，容忍毫秒级边界
}

function verifyCsrfToken(token: unknown, botToken: string, now = Date.now()): boolean {

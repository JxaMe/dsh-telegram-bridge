...
const HELP_TEXT = ('<b>📋 命令帮助</b>\n\n'
    ...
    + '• 处理失败的消息可以点击 🔄 重试');
export function registerTelegramHandlers(bot, runtime) {
    const { hostCtx, api, config, state, pending, settings, sessions, queue, typingEnabled, proxyUrl, debugLog, checkLatestVersion: fetchLatestVersion = checkLatestVersion, } = runtime;
    ...
    bot.command('health', async (ctx) => {
        const m = getMetrics();
        await ctx.reply('✅ dsh-telegram-bridge 运行正常\n'
...
export async function startTelegram(deps) {
    ...
    const errorText = (error) => config.errorDisplayMode === 'friendly'
        ? friendlyError(error)
        : error instanceof Error ? error.message : String(error);
    const queue = new QueueManager(api, sessions, state, async (chatId, error, failureId) => {
        try {
    ...
    }, config.queueLimit ?? 20, config.dataDir ?? process.env.DSH_TELEGRAM_DATA_DIR);
    ...
    void (async () => {
        try {
            ...
            if (info.hasUpdate) {
                await bot.api.sendMessage(config.ownerId, `📢 dsh-telegram-bridge 有新版本 v${info.latest}（当前 v${currentVersion()}）\n更新：${info.url ?? 'https://github.com/JxaMe/dsh-telegram-bridge/releases'}`);
            }
        }
        catch {
...
async function sessionsPanel(state, chatId) {
...
function startTyping(bot, chatId, enabled = true) {
    if (!enabled)
        return;
...
async function runCompact(chatId, ctx, api, state, notify) {
    const chat = state.getChatState(chatId);
    if (!chat) {
        return '当前没有会话。';
    }
...
async function setupCommandMenu(bot, ownerId, botToken, logger) {
    const commands = [

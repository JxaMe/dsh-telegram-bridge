...
async function statusText(chatId, ctx, state, queue) {
    ...
    const provider = settings.provider ?? agent?.options?.provider ?? 'default';
    const model = settings.model ?? agent?.options?.model ?? 'default';
    ...
    const lockText = hasHistory === undefined
        ? settings.agentPreset ? 'unknown' : 'n/a'
        : hasHistory ? 'locked' : 'unlocked';
    const lines = [
        `Busy: ${busy ? 'yes' : 'no'}`,
        ...
        `Preset: ${settings.agentPreset ?? 'default'} (${lockText})`,
        `Messages: ${stats.userMessages} user / ${stats.assistantMessages} assistant`,
        `Tokens: in ${stats.inputTokens}, out ${stats.outputTokens}${stats.cacheReadTokens || stats.cacheWriteTokens ? `, cache ${stats.cacheReadTokens ?? 0}/${stats.cacheWriteTokens ?? 0}` : ''}`,
    ];
    return lines.join('\n');
}

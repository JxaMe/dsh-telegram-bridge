...
export class EventForwarder {
    ...
    statusLineEnabled;
    lastToolAt = new Map();
    constructor(ctx, bot, state, pending, queue, onPendingClear, htmlFormatting = true, statusLineEnabled = true) {
        this.ctx = ctx;
        this.bot = bot;
        this.state = state;
        ...
        this.statusLineEnabled = statusLineEnabled;
    }
    start() {
        this.ctx.on('session/event', async (session, event) => {
            ...
            void this.sendToTelegram(chatId, text).catch((error) => {
                this.onPendingClear?.(chatId);
                this.ctx.logger.warn(`dsh-telegram-bridge 回复发送失败: ${error instanceof Error ? error.message : String(error)}`);
            });
        });
    }
    updateStatusFromEvent(chatId, evt) {
        if (!this.statusLineEnabled)
            return;
        const data = evt.data ?? {};
        switch (evt.type) {
            case 'turn/start':
    ...
    async sendToTelegram(chatId, text) {
        ...
        const isLastChunk = (index) => index === chunks.length - 1 && this.queue.queueLength(chatId) === 0;
...
function splitLongPlainLine(line, limit) {
    ...
    switch (record.type) {
        ...
        case 'table': {
            ...
            if (headers.length === 0 && allRows.length === 0)
                return '';
            const renderRow = (cells) => `| ${cells.map((cell) => escapeHtml(cell)).join(' | ')} |`;
            const lines = [];
            if (headers.length > 0)
